...
class test_seg_net_acc():
    ...
    def initialize_model(self):
        GPU_list = self.GPU
        os.environ["CUDA_VISIBLE_DEVICES"] = GPU_list #str(opt.GPU)  3DUNet  Unet4_no_end_UNet3D
    ...
    def test(self):
        ...
        for im_index in range(0,len(self.name_list)): #opt.test_img_num): #1): #
            ...
            test_SEG_dataloader = DataLoader(test_SEG_data, batch_size=self.SEG_batch_size, shuffle=False, num_workers=0)
            ######################################################################################
