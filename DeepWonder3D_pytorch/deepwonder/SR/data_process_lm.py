...
class trainset_signal_SR_lm(Dataset):
    def __init__(self, img, patch_name_sub_list, coordinate_list):
        self.img = img
        self.patch_name_sub_list = patch_name_sub_list
        self.coordinate_list = coordinate_list

    def __getitem__(self, index):
        patch_name = self.patch_name_sub_list[index]
        single_coordinate = self.coordinate_list[patch_name]
        init_h = single_coordinate['init_h']
        end_h = single_coordinate['end_h']
        init_w = single_coordinate['init_w']
    ...
    def __len__(self):
        return len(self.patch_name_sub_list)
...
####################################################################################################################################
####################################################################################################################################
def train_preprocess_signal_SR_lm(args):
    img_h = args.SR_img_h
    img_w = args.SR_img_w
    img_s = args.SR_img_s
    print('img_s',img_s)
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        ...
        img_path_list[im_name] = im_dir
        img_name_list.append(im_name)
        # name_list.append(im_dir)
        ...
        for ii in range(0, img_num_per_stack):
            ...
            patch_name_sub_list.append(patch_name)
            coordinate_list[patch_name] = single_coordinate
        patch_name_list[im_name] = patch_name_sub_list
    return  patch_name_list, img_path_list, img_name_list, coordinate_list
...
####################################################################################################################################
def train_preprocess_mean_SR_lm(args):
    img_h = args.SR_img_h
    img_w = args.SR_img_w
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        ...
        for ii in range(0, img_num_per_stack):
            ...
            coordinate_list[patch_name] = single_coordinate
    return  name_list, img_path_list, coordinate_list
