    ...
    def __init__(self, UNet_type = '3DUNet', in_channels=1, out_channels=1, frame_num=64, final_sigmoid = True,f_maps=64):
        ...
        if UNet_type == 'TS_UNet3D':
            self.Generator = TS_UNet3D( in_channels = in_channels,
                                     ...
                                     final_sigmoid = final_sigmoid,
                                     f_maps = f_maps)
        if UNet_type == 'Unet4_single_no_end_3D':
            self.Generator = Unet4_single_no_end_3D( in_ch = in_channels,
                                     out_ch = out_channels, 
                                     frame_num = frame_num,
                                     f_num = f_maps)
        if UNet_type == 'Unet4_no_end_UNet3D':
            print('network ---> ',UNet_type)
            self.Generator = Unet4_no_end_3D( in_ch = in_channels,
                                     out_ch = out_channels, 
                                     frame_num = frame_num,
                                     f_num = f_maps)
