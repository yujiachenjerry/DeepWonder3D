...
class testset_SEG(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(np.max(img_patch),' ---> ', np.min(img_patch))
...
def test_preprocess_lessMemory_SEG(args):
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        if '.tif' in im_name:
            ...
            # noise_im = noise_im/np.mean(noise_im)*10 # *10
            noise_im = noise_im/np.max(noise_im)*200
            ...
            assert whole_w>=img_w
            assert whole_h>=img_h
...
###############################
###############################
def test_preprocess_lessMemory_SEG_lm(args):
    img_h = args.SEG_img_h
    img_w = args.SEG_img_w
    img_s2 = args.SEG_img_s
    gap_h = args.SEG_gap_h
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        if '.tif' in im_name:
            ...
            print('whole_w img_w : ',whole_w,img_w)
            del noise_im 
            assert whole_w>=img_w
            assert whole_h>=img_h
